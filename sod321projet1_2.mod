...
continuite1 {i in S1} : sum{j in 1..n}lambda[i,j]=sum{j in 1..n}lambda[j,i];
anticycle {i in 1..n_cycle} : sum {k in 1..n, l in 1..n} M_cycle[i,k + n*(l-1)] * lambda[k,l] <= -1;  #kuah ? je comprends rien à ton inndexation dans M_cycle, et il manque le cardinal de S dans la formule non ?
